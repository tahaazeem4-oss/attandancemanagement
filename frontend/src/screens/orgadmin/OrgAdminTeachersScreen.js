...
const EMPTY_FORM = { first_name: '', last_name: '', email: '', password: '', phone: '', school_id: '' };
...
export default function OrgAdminTeachersScreen({ navigation }) {
  ...
  const openEdit = (item) => {
    setEditing(item);
    setForm({ first_name: item.first_name || '', last_name: item.last_name || '', email: item.email || '', password: '', phone: item.phone || '', school_id: String(item.school_id) || '' });
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await api.put(`/org-admin/teachers/${editing.id}`, { first_name: form.first_name, last_name: form.last_name, email: form.email, phone: form.phone });
      } else {
        await api.post('/org-admin/teachers', { ...form, school_id: parseInt(form.school_id) });
      }
  ...
  const handleDelete = (item) => {
    Alert.alert('Delete Teacher', `Delete "${item.first_name} ${item.last_name}"?`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await api.delete(`/org-admin/teachers/${item.id}`); load(); }
  ...
  const handleResetPassword = async () => {
    ...
    setSaving(true);
    try {
      await api.post(`/org-admin/teachers/${resetModal.id}/reset-password`, { new_password: newPassword });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.topRow}>
        <View style={styles.searchBox}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search teachers..." placeholderTextColor="#94A3B8" value={search} onChangeText={setSearch} />
        </View>
      ...
      <ImportExportBar
        templatePath="/org-admin/import-export/teachers/template"
        importPath="/org-admin/import-export/teachers/import"
        exportPath="/org-admin/import-export/teachers/export"
...
const styles = StyleSheet.create({
  ...
  avatarTxt: { fontWeight: '700', color: '#2563EB', fontSize: 16 },
