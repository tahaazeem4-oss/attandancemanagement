...
function EditStudentModal({ visible, schoolId, student, classes, onClose, onSaved }) {
  ...
  const handleDeleteStudent = async () => {
    setDelLoading(true);
    try {
      await api.delete(`/super-admin/schools/${schoolId}/students/${student.id}`);
      onSaved();
    } catch (e) {
      setError(e?.response?.data?.message || 'Could not delete student.');
      setConfirmDel(false);
    } finally {
      setDelLoading(false);
    }
            ...
            {!confirmDel
              ? (
                <Pressable style={tm.dangerBtn} onPress={() => setConfirmDel(true)}>
                  <Text style={tm.dangerBtnTxt}>Remove Student</Text>
                </Pressable>
              ) : (
                <View>
                  ...
                  <View style={{ flexDirection: 'row', gap: 10 }}>
                    ...
                    <Pressable style={[tm.dangerBtn, { flex: 1 }]} onPress={handleDeleteStudent} disabled={delLoading}>
...
export default function SuperAdminStudentsScreen() {
  ...
  const selectSchool = useCallback((school) => {
    ...
    loadStudents(school.id, null, null);
  }, [loadStudents]);
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <Pressable
                    ...
                    onPress={() => setEditStudent({ open: true, student: item })}
                  >
